...
def filter0(val, class_p):
    return np.array(list(map(lambda v: v if v >= 0.0 else 0.0, val.tolist()))) #* class_p * TOTAL


""" START: Google channel classes """

def google_c1(x):
   return filter0(HIGH / (1 + np.exp(5. - 1 * x)), p_c1)


def google_c2(x):
   return filter0(MEDIUM / (1 + np.exp(5. - 0.8 * x)), p_c2)


def google_c3(x):
   return filter0(LOW / (1 + np.exp(5. - 0.5 * x)), p_c3)
...
def facebook_c1(x):
    return filter0((LOW + SHIFT) / (1 + np.exp(5. - 0.7 * x)), p_c1)

def facebook_c2(x):
    return filter0(MEDIUM / (1 + np.exp(5. - 0.7 * x)), p_c2)

def facebook_c3(x):
    return filter0((LOW + SHIFT) / (1 + np.exp(15. - 1. * x)), p_c3)
...
def instagram_c1(x):
    return filter0(HIGH / (1 + np.exp(6. - 1.3 * x)), p_c1)

def instagram_c2(x):
    return filter0(MEDIUM / (1 + np.exp(12. - 1.3 * x)), p_c2)

def instagram_c3(x):
    return filter0((LOW - 3*SHIFT) / (1 + np.exp(15. - 1.3 * x)), p_c3)
...
def youtube_c1(x):
    return filter0((MEDIUM) / (1 + np.exp(4. - 0.3 * x)), p_c1)

def youtube_c2(x):
    return filter0(LOW / (1 + np.exp(5. - 0.35 * x)), p_c2)

def youtube_c3(x):
    return filter0((LOW - SHIFT) / (1 + np.exp(6. - 0.35 * x)), p_c3)
...
def bing_c1(x):
    return filter0(MEDIUM / (1 + np.exp(4. - 1 * x)), p_c1)

def bing_c2(x):
    return filter0((MEDIUM) / (1 + np.exp(7. - 0.8 * x)), p_c2)

def bing_c3(x):
    return filter0((LOW) / (1 + np.exp(6. - 0.6 * x)), p_c3)
